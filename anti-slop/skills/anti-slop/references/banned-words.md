...
Words whose frequency in published text jumped after 2022, and which corpus work since has repeatedly named as overrepresented in generated prose. `delve` is the most-studied case and the one currently receding fastest, which is a reminder that the list is a snapshot rather than a law. Avoid these in general prose. Do not always pick the first alternative listed; vary replacements across outputs to avoid creating a new detectable pattern.
...
None of these words is bannable, and none of them is on any list in this file. Every one is ordinary English doing an ordinary job most of the time, and a word-level ban would fire on most correctly written prose. The tell is the **metaphorical collocation**, and only when several land in one passage. Treat it exactly as the low-confidence set is treated: a single hit is the writer's prose.
